...
from models import nn, siamois, decomposable_attention, esim, siamois_seq
...
#train = pd.read_csv('train_ngrams.csv')
#test = pd.read_csv('test_edges.csv')
...
#xgb = XGBClassifier(n_estimators=512, max_depth=6)
#cat = CatBoostClassifier()
lgbm = LGBMClassifier(n_estimators=512, max_depth=4, reg_lambda=1., subsample=0.9 )
nn = NeuralNet(nn, batch_size=1024, epochs=40, units=128, dropout=0.3, layers=4)
nnlp = NLPNN(siamois_seq, epochs=15, batch_size=1024, num_words=15000, maxlen=150)
classifier = lgbm
